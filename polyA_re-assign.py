...
syntax = """----------------------------------------------------------------------------------------------------
Usage:	polyA_1.py transcriptome_mod_4.gtf polyA_unused_mod_3.gtf
----------------------------------------------------------------------------------------------------"""
##########################################################################################
if len(sys.argv) != 3:
	print syntax
...
out = open(sys.argv[1].split('.')[0][:-1] + '5.gtf', 'w')
corregidos = []
for chr in trans.keys():
	for strand in trans[chr].keys():
		for tid in trans[chr][strand].keys():
			...
			tlen = ty-tx
			ventana = tlen * 0.20 #--Sólo buscamos al principio de los transcritos (20% de la longitud inicial)
...
trim_out = open (sys.argv[2].split('.')[0][:-1] + '4.gtf', 'w')
i = 0
...
##########################################################################################		
print 'PolyA re-assignment:', len(corregidos)
out_ids = open ('transcripts_mod_5_ids.txt', 'w')
for i in corregidos:
	out_ids.write(i +'\n')
out_ids.close()
##########################################################################################
